using InclusionAnalytics, LinearAlgebra, CairoMakie
...
# Stokes solver
@views function Stokes2D_OOP(n, params, f_anal; γfact = 60 )
    ...
    if dτ_local
        dτVx = 2.0 ./ sqrt.(λmaxVx) * 𝐶
        dτVy = 2.0 ./ sqrt.(λmaxVy) * 𝐶
    else
        dτVx = 2.0 / sqrt(maximum(λmaxVx)) * 𝐶
        dτVy = 2.0 / sqrt(maximum(λmaxVy)) * 𝐶
    end
...
function f_anal(X; params=(ε̇=-1))
    return (V = [X[1]*(-params.ε̇+params.ζ̇), X[2]*(params.ε̇+params.ζ̇)], p=0.0)
end
...
let 
    ...
    fig = Figure()
    ax  = Axis(fig[1,1], aspect=DataAspect(), title=L"$P$")
