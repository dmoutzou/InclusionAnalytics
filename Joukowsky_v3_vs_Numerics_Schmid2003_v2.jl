...
#This is a comparison file between Schmid & Podladchikov (2003) analytics and numerics
...
#Schmid & Podladchikov (2003) analytical solution, incompressible, viscosity ratio mc = ηi/ηm
...
#analytical field (Schmid & Podladchikov, 2003) at PHYSICAL coordinate
#Schmid2003 is incompressible and does not carry a rotation-rate term, so mc = ηi/ηm
#is the only viscosity contrast entering the solution.
f_anal(X; params) = Analytics_Schmid2003(to_zeta(X[1], X[2]); ηm=params.ηm, mc=params.ηi/params.ηm,
                                          γ̇=params.γ̇, ε̇=params.ε̇, α=params.α, t=params.t)
...
function preset(test)
    ...
    elseif test == :Schmid_45deg_ps
        ηm, ηi = 1000.0, 1.0;  ξm, ξi = 1e10, 1e10
...
@views function Stokes2D(n, test; zoom = 3.0, BC_anal=false)
    ...
    γfact    = 6            # penalty: multiplier to the arithmetic mean of η
...
let
    test = :Schmid_ps
    #test = :Schmid_45deg_ps
    #test = :Schmid_30deg_ps
    #test = :Schmid_ss
    #test = :Schmid_3
    ...
    for nc in nc_list
        ...
        save("comparison_JoukowskySchmid2003vsNumerics_nc$(nc)_$(test).png", fig, px_per_unit=2)
        display(fig)
